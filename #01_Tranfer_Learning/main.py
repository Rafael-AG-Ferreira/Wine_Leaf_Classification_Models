...
if userInfo == 0:                       # Caso a escolha do utilizador seja 0 vai ser realizado treino
    ...
    train_data_size = len(data['train'])  # Obtenção da quantidade de dados na pasta train para poder calcular Loss e Accuracy
    valid_data_size = len(data['valid'])  # Obtenção da quantidade de dados na pasta valid para poder calcular Loss e Accuracy
    test_data_size = len(data['test'])    # Obtenção da quantidade de dados na pasta test para poder calcular Loss e Accuracy
    ...
    ComputeTestSetAccuracy(trained_model, loss_func, test_data_loader, test_data_size)  # Testar accuracy e loss para o test dataset
    ...
    resnet50 = torch.load("./Training_History/Grapevine Leaves Classification_model_{}.pt".format(best_epoch))  # Fazer load do Modelo com menores perdas
    ...
    optimizer = optim.SGD(resnet50.parameters(), lr=learning_rate, momentum=0.9)  # Definição do Optimizer para o Fine Tunning
    ...
    scheduler = lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=gamma)  # Agendador das alterações ao Learning Rate
    ...
    fine_tuned_model, ft_history, ft_best_epoch = FineTunningTrainVal(resnet50, loss_func, optimizer, num_epochs_ft,
                                                                      train_data_loader, train_data_size, valid_data_loader,
                                                                      valid_data_size, device, ft_history, scheduler)

    ComputeTestSetAccuracy(fine_tuned_model, loss_func, test_data_loader, test_data_size)  # Testar accuracy e loss para o test dataset
...
if userInfo == 1:                                   # Caso a escolha do utilizador seja 1 vai ser realizada testagem
    ...
    if directTesting:
        ...
        fine_tuned_model = torch.load("./Fine_Tunning_History/Grapevine Leaves Classification - Fine Tunning_model_{}.pt".format(99), map_location=torch.device(device))      # Fine Tuned Model na última época
        ...
        tl_model = torch.load("./Training_History/Grapevine Leaves Classification_model_{}.pt".format(best_epoch), map_location=torch.device(device))
        ft_model = torch.load("./Fine_Tunning_History/Grapevine Leaves Classification - Fine Tunning_model_{}.pt".format(ft_best_epoch), map_location=torch.device(device))
        ...
        trained_model = torch.load("./Training_History/Grapevine Leaves Classification_model_{}.pt".format(99), map_location=torch.device(device))
